...
EOF
    echo -e "${BOLD}${CYAN}  CLI tool for converting various file formats to Markdown${NC}"
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
}
...
get_latest_version() {
    local version
    version=$(curl -s "https://api.github.com/repos/$MQC_REPO/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
get_download_url() {
    ...
    echo "https://github.com/$MQC_REPO/releases/download/$version/mq-conv-${target}${ext}"
}

# Download checksums file
download_checksums() {
    local version="$1"
    local checksums_url="https://github.com/$MQC_REPO/releases/download/$version/checksums.txt"
...
# Download and install mq-conv
install_mqc() {
    local version="$1"
    local os="$2"
    local arch="$3"
    local download_url
    local binary_name="mq-conv"
    local ext=""
    local target=""

    if [[ "$os" == "windows" ]]; then
        ext=".exe"
        binary_name="mq-conv.exe"
        target="${arch}-pc-windows-msvc"
    elif [[ "$os" == "darwin" ]]; then
        target="${arch}-apple-darwin"
    else
    ...
    log "Downloading mq-conv $version for $os/$arch..."
    ...
    if ! curl -L --progress-bar "$download_url" -o "$temp_file"; then
        error "Failed to download mq-conv binary"
    fi

    # Verify checksum
    local release_binary_name="mq-conv-${target}${ext}"
    ...
    mv "$temp_file" "$MQC_BIN_DIR/$binary_name"
    chmod +x "$MQC_BIN_DIR/$binary_name"

    log "mq-conv installed successfully to $MQC_BIN_DIR/$binary_name"
}

# Add mq-conv to PATH by updating shell profile
update_shell_profile() {
    ...
    if echo "$PATH" | grep -q "$MQC_BIN_DIR"; then
        log "$MQC_BIN_DIR is already in PATH"
        return 0
    fi
    ...
    if [[ -n "$shell_profile" ]]; then
        ...
        if [[ "$shell_name" == "fish" ]]; then
            path_export="set -gx PATH \$PATH $MQC_BIN_DIR"
        else
            path_export="export PATH=\"\$PATH:$MQC_BIN_DIR\""
        fi

        if ! grep -q "$MQC_BIN_DIR" "$shell_profile" 2>/dev/null; then
            echo "" >> "$shell_profile"
            echo "# Added by mq-conv installer" >> "$shell_profile"
            echo "$path_export" >> "$shell_profile"
            log "Added $MQC_BIN_DIR to PATH in $shell_profile"
        else
            warn "$MQC_BIN_DIR already exists in $shell_profile"
        fi
    else
        warn "Could not detect shell profile to update"
        warn "Please manually add $MQC_BIN_DIR to your PATH"
    fi
}

# Verify installation
verify_installation() {
    if [[ -x "$MQC_BIN_DIR/mq-conv" ]] || [[ -x "$MQC_BIN_DIR/mq-conv.exe" ]]; then
        log "mq-conv installation verified"
        log "Installation verification successful!"
        return 0
    else
        error "mq-conv installation verification failed"
    fi
}
...
show_post_install() {
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD}${GREEN}mq-conv installed successfully!${NC}"
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo -e "     ${CYAN}mq-conv --version${NC}"
    echo ""
    echo -e "  ${YELLOW}3.${NC} Get help:"
    echo -e "     ${CYAN}mq-conv --help${NC}"
    echo ""
    echo -e "${BOLD}${CYAN}Quick Examples:${NC}"
    echo -e "  ${GREEN}>${NC} ${CYAN}mq-conv document.pdf             # Convert PDF to Markdown${NC}"
    echo -e "  ${GREEN}>${NC} ${CYAN}mq-conv spreadsheet.xlsx         # Convert Excel to Markdown${NC}"
    echo -e "  ${GREEN}>${NC} ${CYAN}mq-conv data.json                # Convert JSON to Markdown${NC}"
    echo ""
    echo -e "${BOLD}${CYAN}Learn More:${NC}"
    echo -e "  ${GREEN}>${NC} Repository: ${BLUE}https://github.com/$MQC_REPO${NC}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            echo "mq-conv installation script"
            echo ""
